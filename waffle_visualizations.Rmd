...
packages("ggplot2", "dplyr", prompt = FALSE)

#https://git.rud.is/hrbrmstr/waffle.git
devtools::install_local("C:\\Users\\czbs7d\\Documents\\R\\packagesGithub\\waffle-master.zip")
library(waffle)
```
...
The content has been copied from the `waffle` [viginette](https://git.rud.is/hrbrmstr/waffle.git). The first example is not part of the viginette.

# Quick Example

Eith visualizations, the trick is to use the right visualization for the right set of data rather than simply using any because of our fondness. Said that, to make Square Pie Charts let’s try to visualize the percentage of Women respondents on [Hackerrank 2018 Developer Survey](https://www.kaggle.com/nulldata/hacker-women-in-tech-hackerrank-analysis)

```{r}
hr_wo <- data.frame(Gender = c("Male","Female"), hr_percentage = c(83,17), stringsAsFactors = FALSE)

ggplot(data = hr_wo, aes(fill = Gender, values = hr_percentage)) +
  geom_waffle(n_rows = 10, size = 0.5, colour = "#ffffff", flip = TRUE) +
  scale_fill_manual(values = c("#f59cbf","#00daff")) +
  coord_equal() + theme_minimal() + theme_enhance_waffle() +
  labs(title = "% Male & Female - Survey Respondents ", subtitle = "Hackerrank Developer Survey 2018",
...
```{r base}
xdf %>% count(states, wt = vals) %>% ggplot(aes(fill = states, values = n)) +
  expand_limits(x=c(0,0), y=c(0,0)) + coord_equal() +
  labs(fill = NULL, colour = NULL) + theme_minimal() + theme_enhance_waffle() -> waf
```

### Plain waffles

```{r plain}
waf +  geom_waffle(n_rows = 20, size = 0.33, colour = "white", flip = TRUE)
```
...
```{r prop}
waf + geom_waffle(n_rows = 10, size = 0.33, colour = "white", flip = TRUE, make_proportional = TRUE)
```
...
```{r bigger-lines}
waf + geom_waffle(n_rows = 10, size = 3, colour = "white", make_proportional = TRUE)
```
...
```{r color-change-1}
waf + geom_waffle(n_rows = 10, size = 0.33, colour = "black", make_proportional = TRUE)
```

We can also "fill in the lines" but that pretty much makes it not a waffle chart:

```{r color-change-2}
waf + geom_waffle(aes(colour = states), n_rows = 10, size = 0.33, make_proportional = TRUE)
```
...
```{r color-change-3}
waf + geom_waffle(aes(color = states), n_rows = 10, size = 0.45, make_proportional = TRUE) +
  scale_colour_manual(values = c(alpha("black", 1/3), "black", alpha("black", 1/3)) )
```
...
```{r color-change-4}
waf + geom_waffle(aes(color = states), n_rows = 10, size = 0.3, make_proportional = TRUE,
...
```{r color-change-5}
waf + geom_waffle(n_rows = 10, size = 0.3, make_proportional = TRUE, height = 0.9, width = 0.9) +
  scale_fill_manual(values = c(alpha("#f8766d", 1/3), "#00ba38", alpha("#619cff", 1/3)) ) 
```
...
```{r round-one}
waf + geom_waffle(n_rows = 10, size = 0.5, color = "white", make_proportional = TRUE,
                  radius = unit(4, "pt") )
```

```{r round-two}
waf + geom_waffle(n_rows = 10, size = 0.5, color = "white", radius = unit(4, "pt") )
```
...
```{r round-three}
waf + geom_waffle(n_rows = 10, size = 1, color = "black", make_proportional = TRUE,
                  radius = unit(4, "pt"), height = 0.8, width = 0.8)
```

You can also use this for the same highlight effect as above:

```{r round-four}
waf + geom_waffle(aes(color = states), n_rows = 10, size = 0.4, make_proportional = TRUE,
                  radius = unit(4, "pt"), height = 0.9, width = 0.9) +
  scale_color_manual(values = c("black", "white", "white"))
```

```{r round-five}
waf + geom_waffle(n_rows = 10, size = 1, color = "white", make_proportional = TRUE,
                  radius = unit(4, "pt"), height = 1, width = 1) +
  scale_fill_manual(values = c("#f8766d", alpha("#00ba38", 1/3), alpha("#619cff", 1/3))  )
```
...
```{r waffle-bar}
waf + geom_waffle(n_rows = 5, color = "white", show.legend = FALSE, flip = TRUE) +
  facet_wrap(~states) + theme_minimal() + theme(panel.spacing.x = unit(0, "npc")) +
...
xdf %>% ggplot(aes(fill = states, values = vals)) + expand_limits(x=c(0,0), y=c(0,0)) +
  coord_equal() + labs(fill = NULL, colour = NULL) + theme_minimal() + theme_enhance_waffle() -> buf
```
...
```{r waffle-buffet}
buf + geom_waffle(color = "white", size = 0.33) +
  facet_wrap(~fct) + theme(strip.text.x = element_text(hjust = 0.5))
```
...
buf + geom_waffle(color = "white", size = 0.33, make_proportional = TRUE, n_rows = 10) +
  facet_wrap(~fct) + theme(legend.position = "bottom") + theme(strip.text.x = element_text(hjust = 0.5))
```

They can be rounded tiles as well:

```{r waffle-buffet-round}
buf + geom_waffle(color = "white", size = 0.33, make_proportional = TRUE, n_rows = 10, radius = unit(2, "pt") ) +
  facet_wrap(~fct) + theme(legend.position = "bottom") + theme(strip.text.x = element_text(hjust = 0.5))
```
...
buf + geom_waffle(color = "white", size = 0.33, make_proportional = TRUE, n_rows = 10, radius = unit(2, "pt") ) +
  facet_wrap(~fct) + scale_fill_manual(values = c("#f8766d", alpha("#00ba38", 1/3), alpha("#619cff", 1/3)) ) +
  theme(legend.position = "bottom") + theme(strip.text.x = element_text(hjust = 0.5))
```
...
buf + geom_waffle(color = "white", size = 0.33, n_rows = 4, flip = TRUE) +
  facet_wrap(~fct) + theme(legend.position = "bottom") + theme(strip.text.x = element_text(hjust = 0.5))
```
...
ggplot(storms_df, aes(fill = status, values = n)) +
  ...
  facet_wrap(~year, nrow = 1, strip.position = "bottom") + scale_x_discrete() +
  scale_y_continuous(labels = function(x) x * 10, # make this multiplyer the same as n_rows
                     expand = c(0,0)) +
  ggthemes::scale_fill_tableau(name=NULL) + coord_equal() +
